 ...
 */
 ...
 /* Intel Q35 Device ID */
 #define PIIX4_DEVICE_ID 0x1237
 #define Q35_DEVICE_ID 0x29c0
 ...
 int unlock_bios_region(void)
 {
    ...
    /* Check for PIIX4 chipset */
    if ((host_bridge_id & 0xFFFF) == INTEL_VENDOR_ID &&
        ((host_bridge_id >> 16) & 0xFFFF) == PIIX4_DEVICE_ID) {
        if (unlock_piix4_pam() == 0) {
            return 0;  /* Success */
        }
    }

     /* Check for Q35 chipset */
     if ((host_bridge_id & 0xFFFF) == INTEL_VENDOR_ID &&
         ((host_bridge_id >> 16) & 0xFFFF) == Q35_DEVICE_ID) {
         if (unlock_q35_pam() == 0) {
             return 0;  /* Success */
         }
     }

     /* Check for Intel chipset (Skylake+) */
     if ((host_bridge_id & 0xFFFF) == INTEL_VENDOR_ID) {
         if (unlock_skylake_pam() == 0) {
             return 0;  /* Success */
         }
     }
     
     printf("Failed to unlock BIOS region: Unsupported chipset\n");
     return -1;  /* Failure */
 }
